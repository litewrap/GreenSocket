...
public class Socket: SocketReader, SocketWriter {
    ...
    #if os(Windows)
    public static var INADDR_ANY                             = in_addr()
    #else
    ...
    // GreenSocket addition
    public static let SOCKET_ERR_WINSOCK_INIT_ERROR         = -9963
    public static let SOCKET_ERR_WINSOC_IOCTLSO_IOCTLSOCKET = -9962
    ...
    public enum SocketProtocol: Int32 {
        ...
        var value: Int32 {
            ...
            case .tcp:
                #if os(Windows)
                    return Int32(IPPROTO_TCP.rawValue)
                #else
        ...
        static func getProtocol(forValue: Int32) -> SocketProtocol? {
            ...
            #if os(Windows)
            case Int32(IPPROTO_TCP.rawValue):
                return .tcp
            case Int32(IPPROTO_UDP.rawValue):
                return .udp
            case Int32(0):
                return .unix
            default:
    ...
    public struct Signature: CustomStringConvertible {
        ...
        public init?(socketType: SocketType, proto: SocketProtocol, path: String?) throws {
            ...
            #if os(Windows)
            remoteAddr.sun_family = UInt16(WinSDK.AF_UNIX)
            #else
            ...
            #elseif os(Windows)
                // TBD WINDOWS
                //remoteAddr.sun_len = UInt8(MemoryLayout<UInt8>.size + MemoryLayout<UInt16>.size + path.utf8.count + 1)
    ...
    internal let uuid = Foundation.UUID()
    ...
    public class func hostnameAndPort(from address: Address) -> (hostname: String, port: Int32)? {
        ...
        case .ipv4(let address_in):
            ...
            #if os(Windows)
                inet_ntop(Int32(addr_in.sin_family), &addr_in.sin_addr, &buf, Int(socklen_t(bufLen)))
            #else
        ...
        case .ipv6(let address_in):
            ...
            buf = [CChar](repeating: 0, count: bufLen)
            #if os(Windows)
                inet_ntop(Int32(addr_in.sin6_family), &addr_in.sin6_addr, &buf, Int(socklen_t(bufLen)))
            #else
    ...
    private init(family: ProtocolFamily, type: SocketType, proto: SocketProtocol) throws {
        ...
        #elseif os(Windows)
            self.socketfd = Int32(WinSDK.socket(family.value, type.value, sockProto.value))
        #else
        ...
        if path != nil {
            ...
            if let (hostname, port) = Socket.hostnameAndPort(from: remoteAddress) {
                ...
                try self.signature = Signature(
                    ...
                    socketType: type,
                    proto: Int32(IPPROTO_TCP.rawValue),
        ...
        repeat {
            do {
                guard let acceptAddress = try Address(addressProvider: { (addressPointer, addressLengthPointer) in
                    ...
                    #elseif os(Windows)
                        let fd = Int32(WinSDK.accept(SOCKET(self.socketfd), addressPointer, addressLengthPointer))
                    #else
    ...
    public func connect(to host: String, port: Int32, timeout: UInt = 0, familyOnly: Bool = false) throws {
        ...
        while info != nil {
            ...
            #elseif os(Windows)
                socketDescriptor = Int32(WinSDK.socket(info!.pointee.ai_family, info!.pointee.ai_socktype, info!.pointee.ai_protocol))
            #else
            ...
            if !self.isBlocking || timeout > 0 {
                ...
                let cmd: Int32 =  WinSDK.FIONBIO
                let result = WinSDK.ioctlsocket(SOCKET(socketDescriptor!), cmd, &mode)

                if result == WinSDK.SOCKET_ERROR {
                    throw Error(code: Socket.SOCKET_ERR_WINSOC_IOCTLSO_IOCTLSOCKET, reason: "ioctlsocket failed for non-blocking mode")
                }
            ...
            #elseif os(Windows)
                status = Int32(WinSDK.connect(SOCKET(socketDescriptor!), info!.pointee.ai_addr, Int32(info!.pointee.ai_addrlen)))
            #else
            ...
            if errno == EINPROGRESS {
                ...
                if timeout > 0 {
                    ...
                    if writefds.isSet(socketDescriptor!) {
                        ...
                        try withUnsafeMutablePointer(to: &result) { result_ptr in
                            try result_ptr.withMemoryRebound(to: Int8.self, capacity: 1) { int8_result_ptr in
                                //getsockopt(s: SOCKET, level: Int32, optname: Int32, optval: UnsafeMutablePointer<CChar>!, optlen: UnsafeMutablePointer<Int32>!)
                                
                                if getsockopt(SOCKET(socketDescriptor!), SOL_SOCKET, SO_ERROR, int8_result_ptr, &resultLength) < 0 {
        ...
        if self.isBlocking && timeout > 0 {
            ...
            var mode: UInt32 = 1 // non-blocking
            let cmd: Int32 =  WinSDK.FIONBIO
            let result = WinSDK.ioctlsocket(SOCKET(self.socketfd), cmd, &mode)
    ...
    public func connect(to path: String) throws {
        ...
        let rc = addrPtr.withMemoryRebound(to: sockaddr.self, capacity: 1) {
            ...
            #elseif os(Windows)
                return WinSDK.connect(SOCKET(self.socketfd), p, socklen_t(addrLen))
            #else
    ...
    public func connect(using signature: Signature) throws {
        ...
        if let address = signature.address {
            ...
            let rc = address.withSockAddrPointer { sockaddr, length -> Int32 in
                ...
                #elseif os(Windows)
                    return WinSDK.connect(SOCKET(self.socketfd), sockaddr, length)
                #else
    ...
    public func listen(on port: Int, maxBacklogSize: Int = Socket.SOCKET_DEFAULT_MAX_BACKLOG, allowPortReuse: Bool = true, node: String? = nil) throws {
        ...
        // Configure ipv6 socket so that it can share ports with ipv4 on the same port.
            ...
            try withUnsafePointer(to: &on) { on_ptr in
                ...
                if sig.protocolFamily == .inet6 && sig.proto == .tcp {
                    if setsockopt(SOCKET(self.socketfd), Int32(IPPROTO_IPV6.rawValue), IPV6_V6ONLY, int8_on_ptr, socklen_t(MemoryLayout<Int32>.size)) < 0 {
        ...
        while info != nil {
            ...
            #elseif os(Windows)
                if WinSDK.bind(SOCKET(self.socketfd), info!.pointee.ai_addr, Int32(info!.pointee.ai_addrlen)) == 0 {
        ...
        if port == 0 {
            guard let addressFromSockName = try Address(addressProvider: { (sockaddr, length) in
                #if os(Windows)
                if getsockname(SOCKET(self.socketfd), sockaddr, length) != 0 {
        ...
        #elseif os(Windows)
            if WinSDK.listen(SOCKET(self.socketfd), Int32(maxBacklogSize)) < 0 {
    ...
    @discardableResult public func write(from buffer: UnsafeRawPointer, bufSize: Int) throws -> Int {
        ...
        while sent < bufSize {
            ...
            if self.delegate != nil {
                ...
                #elseif os(Windows)
                    s = Int(WinSDK.send(SOCKET(self.socketfd), buffer.advanced(by: sent), Int32(bufSize - sent), sendFlags))
                #else
    ...
    @discardableResult public func write(from buffer: UnsafeRawPointer, bufSize: Int, to address: Address) throws -> Int {
        ...
        return try address.withSockAddrPointer { addressPointer, addressLength -> Int in
            ...
            while sent < bufSize {
                ...
                #elseif os(Windows)
                    s = Int(WinSDK.sendto(SOCKET(self.socketfd), buffer.advanced(by: sent), Int32(bufSize - sent), sendFlags, addressPointer, addressLength))
                #else
    ...
    public func setBlocking(mode shouldBlock: Bool) throws {
        ...
        var result: Int32 = 0
         if shouldBlock {
            ...
            result = WinSDK.ioctlsocket(SOCKET(self.socketfd), cmd, &mode)
        } else {
            var mode: UInt32 = 1 // non-blocking
            result = WinSDK.ioctlsocket(SOCKET(self.socketfd), cmd, &mode)
        }
    ...
    public func setReadTimeout(value: UInt = 0) throws {
        ...
        #if os(Windows)
        try withUnsafePointer(to: &timer) { on_ptr in
            try on_ptr.withMemoryRebound(to: Int8.self, capacity: 1) { int8_timer_ptr in
                let result = setsockopt(SOCKET(self.socketfd), SOL_SOCKET, SO_RCVTIMEO, int8_timer_ptr, socklen_t(MemoryLayout<timeval>.stride))
                if result < 0 {
    ...
    public func setWriteTimeout(value: UInt = 0) throws {
        ...
        try withUnsafePointer(to: &timer) { on_ptr in
            try on_ptr.withMemoryRebound(to: Int8.self, capacity: 1) { int8_timer_ptr in
                let result = setsockopt(SOCKET(self.socketfd), SOL_SOCKET, SO_SNDTIMEO, int8_timer_ptr, socklen_t(MemoryLayout<timeval>.stride))
    ...
    public func udpBroadcast(enable: Bool) throws {
        ...
        try withUnsafePointer(to: &on) { on_ptr in
            try on_ptr.withMemoryRebound(to: Int8.self, capacity: 1) { int8_on_ptr in
                if setsockopt(SOCKET(self.socketfd), SOL_SOCKET, SO_BROADCAST, int8_on_ptr, socklen_t(MemoryLayout<Int32>.size)) < 0 {
                    throw Error(code: Socket.SOCKET_ERR_SETSOCKOPT_FAILED, reason: self.lastError())
                }
            }
        }
    ...
    private func close(withSSLCleanup: Bool) {
        ...
        if self.socketfd != Socket.SOCKET_INVALID_DESCRIPTOR {
            ...
            #elseif os(Windows)
                if self.isListening {
                    _ = WinSDK.shutdown(SOCKET(self.socketfd), 2)
                    self.isListening = false
                }
                self.isConnected = false
                _ = WinSDK.closesocket(SOCKET(self.socketfd))
            #else
                if self.isListening {
        ...
        if let _ = self.signature {
            ...
            if self.signature!.path != nil && self.isListening {
                ...
                #elseif os(Windows)
                    _ = WinSDK._unlink(self.signature!.path!)
                #else
    ...
    private func readDataIntoStorage() throws -> Int {
        ...
        repeat {
            ...
            if self.delegate == nil {
                ...
                #elseif os(Windows)
                    count = Int(WinSDK.recv(SOCKET(self.socketfd), self.readBuffer, Int32(self.readBufferSize), recvFlags))
                #else
    ...
    private func readDatagramIntoStorage() throws -> (bytesRead: Int, fromAddress: Address?) {
        ...
        do {
            guard let address = try Address(addressProvider: { (addresssPointer, addressLengthPointer) in
                ...
                #elseif os(Windows)
                    let count = Int(WinSDK.recvfrom(SOCKET(self.socketfd), self.readBuffer, Int32(self.readBufferSize), recvFlags, addresssPointer, addressLengthPointer))
                #else
    ...
    private func winsockInit() throws {
        #if os(Windows)
